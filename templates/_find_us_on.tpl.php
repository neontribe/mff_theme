            <div class="clr">
              <? if ($_SERVER['SERVER_NAME']== "www.makefruitfair.de" OR $_SERVER['SERVER_NAME']== "www.frutasjustas.org" ) { ?>
               <div class="social_de">
              <?  } else { ?>
               <div class="social">
              <?  }?>                           
                <h5><?= t('Follow us on') ?>:</h5>
                <div class="clr ico">
                  <?php if($language == 'cz' || $language == 'fr' || $language == 'en'):?>
                  <a href="<?=$facebook_link?>"><img src="<?=$image_path?>/ico/facebook.gif" alt="" /></a>
                  <?php endif;?>
                  
                 <?php if($language == 'fr' || $language == 'en' || $language == 'cz'):?>
                  <a href="<?=$twitter_link?>"><img src="<?=$image_path?>/ico/twitter.gif" alt="" /></a>
                  <?php endif;?>      
                  
                  <a href="/rss.xml"><img src="<?=$image_path?>/ico/rss.gif" alt="" /></a>
                  <a href="http://www.flickr.com/photos/makefruitfair"><img src="<?=$image_path?>/ico/flickr.gif" alt="" /></a>
                  <a href="http://www.youtube.com/user/makefruitfair"><img src="<?=$image_path?>/ico/youtube.gif" alt="" /></a>
                </div>
              </div>
              <? if ($_SERVER['SERVER_NAME']== "www.makefruitfair.de" OR $_SERVER['SERVER_NAME']== "www.frutasjustas.org" ) { ?>
               <div class="paypal_de">
              <?  } else { ?>
              <div class="paypal">
              <?  }?>                 
                <h5><?= t('Donate') ?></h5>
                <?php if($language == 'fr'):?>
                <a href="http://www.peuples-solidaires.org/ce-que-vous-pouvez-faire/don-en-ligne/"><img src="<?=$image_path?>/partner/peuples3.gif" alt="" /></a>
                <?php elseif($language == 'de'):?>
                <p><a href="http://www.banafair.de/spenden">online spenden</a></p>
                <?php elseif($language == 'cz'):?>
                <form id="paypalDonate" action="https://www.paypal.com/cgi-bin/webscr" method="post"> <input name="cmd" value="_s-xclick" type="hidden"> <input name="hosted_button_id" value="TY8TE34SM3NEY" type="hidden"> <input alt="PayPal - The safer, easier way to pay online!" name="submit" src="<?=$image_path?>/ico/paypal.png" type="image"> <img src="https://www.paypal.com/en_US/i/scr/pixel.gif" alt="" border="0" height="1" width="1"> </form>
                <?php else:?>
<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
<input type="hidden" name="cmd" value="_s-xclick">
<input type="image" src="<?=$image_path?>/ico/paypal.png" border="0" name="submit" alt="Make payments with PayPal - it's fast, free and secure!">
<img alt="" border="0" src="https://www.paypal.com/en_GB/i/scr/pixel.gif" width="1" height="1">
<input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----
">
</form>
                <?php endif;?>
              </div>
            </div>
